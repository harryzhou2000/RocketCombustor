!!------------------------------------------------------------------
!! BDF2 method, using dual time stepping approach
!!------------------------------------------------------------------
subroutine BDF2_stage(istep)
    use mainvar
    use Parallel_Var
    implicit none
    integer:: inner_rk,istep
    double precision:: RoRes, RoRes_tmp, phi
    ! integer,save:: iuse=0
    ...
    subroutine BDF2_LU_SGS(Inner_rk,phi)
    use mainvar
    use species
    implicit none
    integer :: inner_rk
    ...
    if( inner_rk==1 )then  
      do i=1,NE
        fixed(:,i)=vol(i)/dt*((1+2*phi)*WP(:,i)-phi*WP2(:,i))
      enddo
    endif
    ...
    enddo
    dd(i)= dd(i)+ vol(i)*(1./step(i)+ (1+phi)/dt)
    enddo
    ...
    do k=1, Nvar
      ...
      Res= hWA(k,i)- (1+phi)*vol(i)/dt*WA(k,i)+fixed(k,i)
      hWA(k,i)= Res
    enddo
    enddo
    ...
    do i=1, NE
      ...
      do j=1,Nvar-Nflow
        JacoMat(nflow+j,:)= JacoMat(nflow+j,:) - vol(i)*dwdp(j,:)
      enddo
