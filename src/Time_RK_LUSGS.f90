...
!!------------------------------------------------------------------
subroutine irk_stage(istep,rk)
    use mainvar
    use Parallel_Var
    implicit none
    integer:: rk,inner_rk,istep
    ...
    do inner_rk=1,InnerLoop
      ...
      if( isnan(WA(1,1)) )then
        write(*,*) 'there is NAN in step ',istep,'stage',rk,'innerloop',inner_rk,'in processor', MyProc
        call MPI_STOP
      endif

    enddo

    if(rk==InLoop) call updatevariables_irk

    return
end subroutine

    subroutine IRK_LU_SGS(rk,Inner_rk)
    use mainvar
    use species
    implicit none
    integer :: rk,inner_rk
    ...
    !! backup
    if( rk==1 .and. inner_rk==1 )then
      do i=1,NE
        WP (:,i)= WA (:,i)
      enddo
    endif
    
    if(inner_rk==1)then           !! at the start of an inner iteration
    do i=1,NE
      fixed(:,i)=vol(i)/dt*WP(:,i)
      do it=1,rk-1
      fixed(:,i)=fixed(:,i)+asj(rk,it)*hWAP(it,:,i)
      enddo
    enddo
    endif
    ...
    do nei=1,KFACE(i)  !! Tri/Quad= 3/4
      iface= NeighF(nei,i)
      dd(i)= dd(i)+ asj(rk,rk)*0.5*alpaF(iface)
    enddo
    ...
    do k=1, Nvar
      ...
      Res= asj(rk,rk)*hWA(k,i)- vol(i)/dt*WA(k,i)+fixed(k,i)
      hWA(k,i)= Res
    enddo
    enddo
    ...
    do i=1, NE
      ...
      do j=1,Nvar-Nflow
        JacoMat(nflow+j,:)= JacoMat(nflow+j,:) - asj(rk,rk)*vol(i)*dwdp(j,:)
      enddo
...
subroutine updatevariables_irk
  use mainvar
  implicit none
  integer:: i,k
  ...
  do i=1,NE
  do k=1,Nvar
    WA(k,i)=WP(k,i)+dt/vol(i)*sum(bs(1:InLoop)*hWAP(1:InLoop,k,i))
  enddo
  enddo
