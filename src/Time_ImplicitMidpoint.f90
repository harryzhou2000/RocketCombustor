...
!!------------------------------------------------------------------
subroutine implicitMidpoint_stage(istep)
    use mainvar
    use Parallel_Var
    implicit none
    ...
    do inner_rk=1,InnerLoop
      ...
      ! stop
      call implicitMidpoint_LU_SGS(Inner_rk)
    ...
    subroutine implicitMidpoint_LU_SGS(Inner_rk)
    use mainvar
    use species
    implicit none
    integer :: inner_rk
    ...
    enddo
    dd(i)= dd(i)+ 2*vol(i)*(1./step(i)+ 1./dt)
    enddo
    ...
    do k=1, Nvar
      ...
      Res= hWA(k,i)- vol(i)/dt*WA(k,i)+fixed(k,i)
      hWA(k,i)= Res
    enddo
    enddo
